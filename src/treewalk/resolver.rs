...
impl VarScopes {
    ...
    fn pop(&mut self) {
        self.stack.pop().expect("Attempted to pop empty VarScopes stack."); // panic here because this would indicate a bug in the interpreter
    }
...
fn function_statement(name: &Token, _params: &[Token], body: &Stmt, scope: &mut VarScopes) -> UnitResult {
...
fn resolve_expr(expression: &Expr, scope: &mut VarScopes) -> UnitResult {
    match expression {
        ...
        Expr::Binary { left, operator, right } => resolve_binary(left, operator, right, scope),
        Expr::Call {
            callee,
            paren,
            arguments,
        ...
        Expr::Logical { left, operator, right } => resolve_logical(left, operator, right, scope),
...
fn resolve_binary(left: &Expr, _operator: &Token, right: &Expr, scope: &mut VarScopes) -> UnitResult {
    resolve_expr(left, scope)?;
    resolve_expr(right, scope)
}

fn resolve_call(callee: &Expr, _paren: &Token, arguments: &[Expr], scope: &mut VarScopes) -> UnitResult {
...
fn resolve_logical(left: &Expr, _operator: &Token, right: &Expr, scope: &mut VarScopes) -> UnitResult {
    resolve_expr(left, scope)?;
    resolve_expr(right, scope)
}
